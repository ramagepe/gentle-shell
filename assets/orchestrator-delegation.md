...
For generic non-SDD research that needs external evidence, first attempt the installed package-owned `gentle-ai-research`. The parent provides questions, scope, depth, source restrictions, and fresh `research_selection`; the child remains non-nesting and read-only, with only `read`, `grep`, and `find` locally plus selected/provenanced approved web tools. MCP, Bash, writes, memory, installs, authentication, credentials, and publication remain excluded. If the role is missing or unusable, fall back to Pi's native `Agent` with equivalent constraints and report the fallback. Never substitute `sdd-research`, which remains SDD-only.
