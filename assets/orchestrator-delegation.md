...
These instructions apply to organic work, not explicitly selected SDD. Preserve the existing direct/delegated topology and one parent owner; do not introduce an ODD phase CLI or execution harness. Package-owned bounded delegation roles remain ordinary routing, not new workflow phases.
...
Return concise findings, recommendation, tradeoffs, open questions, and implementation implications. Offer a concise proposal only when a real scope or product decision needs it. Neither research nor a proposal is mandatory. Forward these research instructions to the package-owned `gentle-ai-research` role for generic non-SDD external evidence. It may use only fixed local reads plus explicitly selected, provenance-bound web tools; `sdd-research` remains SDD-only. If the role is missing or unusable, use the native `Agent` fallback with equivalent read-only and tool constraints and report the fallback. Research remains read-only and requires no new persistence or readiness machinery.
...
For generic non-SDD research that needs external evidence, first attempt the installed package-owned `gentle-ai-research`. The parent provides questions, scope, depth, source restrictions, and fresh `research_selection` containing source classes and semantic tool names only; it never supplies hidden extension paths. The host derives trusted extension provenance from its active registered non-SDK inventory, and the child revalidates it locally. The child remains non-nesting and read-only, with only `read`, `grep`, and `find` locally plus selected/provenanced approved web tools. MCP, Bash, writes, memory, installs, authentication, credentials, and publication remain excluded. If the role is missing or unusable, fall back to Pi's native `Agent` with equivalent constraints and report the fallback. Never substitute `sdd-research`, which remains SDD-only.
